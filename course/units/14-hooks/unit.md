...
> its output. The thing that makes a hook different from a command/skill/subagent: **the
...
event names and the exact shape are the version-specific surface — confirm them against the docs rather than authoring from memory.
...
**This course enforces its own standards with the very mechanism this unit teaches.** Open this
...
- **The policy isn't duplicated in the hook.** The hook *calls* the existing `make check`; it doesn't
  re-implement any rule. The standard lives in one place; the hook is just a new *trigger* for it.
...
event name and structure against the docs / the settings schema rather than memory.
...
- **Authoring event names from memory.** The event enum is version-specific and large; a guessed name
  just never fires. Confirm against the settings schema / docs.

## Going deeper

- **Next:** **MCP & vetting** connects external tools and asks how far to trust them; **Automate &
