...
**User story:** As a learner (and the course maintainer), I want an honest, multi-perspective retrospective of how this course was actually built with Claude Code — what the human author and Claude each did well, did okay on, and could improve, across both process and human-language communication — produced by a panel of subjective persona reviewers reading the real session transcripts, so that the course teaches effective human+Claude co-authorship from a candid worked example rather than an idealized one. *(Added post-v1, 2026-05-31; anchors additively to R14 (dogfooding), R10 (responsible & verified use), R3 (workflow methods), and R8 (capstone exemplar).)*

**Acceptance criteria**
- R18.AC1 — THE COURSE SHALL produce a **collaboration retrospective** evaluating its own build, assessing **both parties** — the human author and Claude — along two explicit axes: **process/workflow** and **human-language communication**.
- R18.AC2 — Each per-session × per-reviewer evaluation SHALL be a **substantive, verbose written assessment** — not a selection among fixed categories. Through its persona's lens it SHALL surface the session's **key, important insights**: concrete notable moments, recurring patterns, decisive choices and turning points, effective habits worth repeating, and missed opportunities — for **both** parties, each **explained with reasoning** and **ordered by significance**, at whatever length the evidence warrants. It SHALL address both strengths and weaknesses rather than being one-sided.
- R18.AC3 — A concise three-level grade — **did well / did okay / could improve** — SHALL be retained as an at-a-glance **summary verdict** (per party, per axis, and overall) that makes each verbose assessment scannable; the grade SHALL **summarize, never replace,** the narrative insights of R18.AC2.
- R18.AC4 — The retrospective SHALL be produced by a **panel of distinct persona reviewers**, each implemented as a **real, committed Claude Code subagent** (`.claude/agents/`) focused on a key element of effective co-authorship / effective CLI use; the panel SHALL be a genuine dogfooded artifact (R14.AC1–AC3), not a contrived prop, and SHALL be referenced as a worked example from **U13** (subagents) — for which the repo previously had no authentic subagent to show (decision P5-U13-example).
- R18.AC5 — Every reviewer SHALL render **subjective, candid judgments** regardless of its persona: it SHALL NOT flatter, soften to please, or discourage, and SHALL aim for accuracy and thoroughness. Each insight SHALL be **grounded in cited transcript evidence** (session + locator) and SHALL NOT be fabricated or asserted from memory (consistent with R10.AC9 and the R12.AC3 verify-don't-assume discipline).
- R18.AC6 — The evaluation SHALL be structured as a **session × reviewer matrix**, summarized along both margins and then to a bottom line: (a) **leaf** — a per-session × per-reviewer evaluation considering only that one session, persisted ("cached") as a committed artifact; (b) **per-session synthesis** — consolidating every reviewer's evaluation of that one session (the reviewer-axis margin); (c) **per-reviewer global** — each reviewer's own synthesis across **all** sessions, built from its own leaves (the session-axis margin); and (d) an **overall global synthesis** consolidating the per-reviewer globals into a bottom line. Each synthesis SHALL be derived from — and traceable to — the artifacts beneath it, and SHALL itself carry both the verbose cross-cutting insights (R18.AC2) and the summary grades (R18.AC3).
- R18.AC7 — The retrospective SHALL **attribute the AI side accurately, per session**, to the model that actually produced it (verified from the transcript record, never assumed — e.g., the foundational requirements/design session is the lone **mixed** case, its first **2** assistant turns on `claude-sonnet-4-6` then `claude-opus-4-8` for the rest, while every other session ran entirely on `claude-opus-4-8`), so that no finding misattributes behavior to a model that did not produce it (R10.AC9 honesty; R12.AC3 philosophy).
- R18.AC8 — The retrospective SHALL be delivered as **both** a learner-facing **case study** (in `course/`, cross-referenced as capstone-exemplar material per R8.AC2 and R14.AC4) **and** a first-class internal **evaluation corpus** (the cached per-session × per-reviewer artifacts plus the per-session and global syntheses), each maintained as a real, accurate deliverable.
- R18.AC9 — The build of this retrospective SHALL itself follow the **spec-driven, gated process** the course teaches (R3.AC2) via the post-v1 enhancement playbook (R13.AC3), and be referenced as a dogfooding example (R14.AC2); the requirement (this R18) SHALL be enforced by the existing dynamically-discovered traceability check (R13.AC5) with **no special-casing**.
- R18.AC10 — The retrospective SHALL keep its references to the source material **accurate to the repository's actual `log/transcripts/` contents** (R14.AC6, enforced by R13.AC4) and SHALL introduce **no new secrets**, relying on the capture workflow's existing secret-scan gate.

### R19 — Breadcrumb navigation for learner-facing documents
**User story:** As a learner moving through the course, I want a breadcrumb trail at the top of every learner-facing document showing where it sits and linking back to its parent document(s) up to the course home, so I never lose my place or have to rely on the browser/editor back button to climb the hierarchy. *(Added post-v1, 2026-05-31; anchors additively to R15 (accessibility & navigability), R13 (maintainability & enforcement), and R6 (unit standard / committed-rendered pattern).)*

**Acceptance criteria**
- R19.AC1 — Every **learner-facing document** SHALL carry a **breadcrumb trail at its top** that links back through its **parent document(s)** up to the course home. In scope: course units and their labs, the capstone briefs/exemplar/rubric and case studies (R18), the CLI reference, the progress checklist, and the "when you're stuck" guide, plus the learner-facing `README`. Out of scope: maintainer-/contributor-facing docs (e.g., the maintainer guide), the spec, and `meta/` artifacts.
- R19.AC2 — Each ancestor breadcrumb segment SHALL be a **working relative link** that resolves to an existing document (enforced by the link check, R13.AC4); the final segment SHALL identify the **current** document and need not be a link.
- R19.AC3 — Breadcrumbs SHALL reflect the document's **actual place in the course hierarchy** (consistent with the course navigation index, R15), and their **placement and format SHALL be consistent across all in-scope documents** and recorded in `meta/conventions.md` so new documents adopt the same pattern (R13.AC1).
- R19.AC4 — Breadcrumbs SHALL be authored in **CommonMark** and degrade gracefully (R15.AC2): the trail SHALL be readable as plain text/links in any Markdown viewer, and its meaning SHALL NOT depend on color, emoji, or non-semantic formatting alone (R15.AC6).
- R19.AC5 — Breadcrumbs SHALL be **enforced mechanically** by the check suite (R13.AC4): a check SHALL fail on a missing, stale, or hierarchy-mismatched trail rather than let it rot silently. WHERE a document is generated (e.g., units' `unit.src.md`→`unit.md` via `tools/render-units`, R6/P7), the breadcrumb SHALL be **single-sourced and emitted by that generator** so it refreshes automatically (R12.AC8 quarantine philosophy).

### R20 — Show, don't tell: demonstration artifacts in unit content
**User story:** As a learner who is a seasoned, skeptical engineer new to AI-assisted coding, I want the units to *show* me the tool being used — what a plan looks like, what reading a diff looks like, what Claude's replies look like and how an operator steers them — not only describe the discipline in prose, so I can recognize and imitate the skill instead of reconstructing it from assertions. *(Added post-v1, 2026-06-09; motivated by the five-reviewer content review in `log/content-review/` — converged finding S1: zero diffs, zero example plans, and one simulated reply across ~3,300 lines of unit prose. Anchors additively to R6 (unit standard), R5 (audience fit), R14 (authentic dogfooding), R13 (enforcement), R12 (version quarantine), and R15 (accessibility).)*

**Acceptance criteria**
- R20.AC1 — Every **core-tier unit** SHALL demonstrate its primary skill with at least one **concrete interaction artifact** — an annotated example of the actual exchange (e.g., a realistic plan-mode plan, a diff being read with the reviewer's observations, failing/passing test output, a session-transcript excerpt with operator commentary) — in addition to, never replaced by, prose description.
- R20.AC2 — The skills the course itself names as central SHALL each be **shown at least once** somewhere in the course: a plan being reviewed (including a **weak plan and the redirect message that fixes it**), a diff being read before acceptance, test output being interpreted (red for the right reason, green-but-insufficient), and a multi-turn steering exchange. A unit telling the learner to reject vague plans WHILE no vague plan is ever shown SHALL be treated as a defect of this requirement.
- R20.AC3 — Each unit's **"Worked example" section SHALL work an example of the unit's skill as an interaction**; provenance/dogfooding references (R14) MAY complement but SHALL NOT substitute for demonstration (closing the six provenance-only sections found in the review).
- R20.AC4 — Illustrative artifacts SHALL be **clearly marked as illustrative** and **non-determinism-safe** — stating that the learner's output will differ in wording and that behavior/diffs, not phrasing, are what to verify (R5/R10 philosophy) — and SHALL NOT assert version-specific surface from memory: any version-specific detail inside a demonstration SHALL be referenced via the version-data mechanism (R12.AC2/AC3) or shown as captured output with provenance (R12.AC4).
- R20.AC5 — The demonstration convention (callout form, marking, placement) SHALL be **recorded in `meta/conventions.md`** (R13.AC1), and presence SHALL be **enforced mechanically** by the check suite (R13.AC4): a core unit lacking a conforming demonstration artifact SHALL fail the check rather than rot silently.
- R20.AC6 — Demonstration artifacts SHALL degrade gracefully as CommonMark text (R15.AC2): no meaning carried by color/emoji alone (R15.AC6), and any diagram SHALL carry a text equivalent (R15.AC5).

### R21 — Operator craft, consolidation, and transfer
**User story:** As a learner who wants to reach genuinely elite practice, I want the course to teach the operator half of the craft — composing and iterating my own briefs, managing a session's context lifecycle, and recovering a derailed session — and to help me consolidate skills across stages and transfer them to my own repository, so I finish an operator rather than a follower of pre-written prompts. *(Added post-v1, 2026-06-09; motivated by the content review's converged findings S3/S5 in `log/content-review/`. Anchors additively to R5 (audience), R9 (solo survival), R7.AC8 (BYO labs), and R1 (taxonomy unchanged — taught within existing can-dos, decision P11-taxonomy).)*

**Acceptance criteria**
- R21.AC1 — At least one core unit SHALL have the learner **author their own brief/prompt, observe the result, critique it, and revise it** (a compose→observe→revise loop), rather than only executing prompts the course wrote; the unit content SHALL teach what distinguishes a brief that works (scope, constraints, deliverable, context) using a weak/strong contrast (R20.AC2 form).
- R21.AC2 — THE COURSE SHALL teach **session-lifecycle judgment**: recognizable signs of context degradation, choosing between continuing, compacting, clearing, and restarting, and **resuming work across sessions** — taught in its home unit and **reinforced in at least one long-form lab** elsewhere in the course (not a single unreinforced paragraph).
- R21.AC3 — THE COURSE SHALL teach **recognizing and recovering a derailed session** — when to interrupt, redirect, roll back, or start over — cross-referenced from the "when you're stuck" guide (R9.AC4) so the recovery content is reachable at the moment of need.
- R21.AC4 — Each **stage boundary** (First Wins → Daily Driver → Force Multiplier → Autonomy & Scale) SHALL provide a brief **consolidation checkpoint**: a recap with retrieval-style prompts the learner answers from memory before moving on (ungraded, honoring the capstone-only assessment, R8/D0.3).
- R21.AC5 — THE COURSE SHALL provide **transfer exercises** that direct the learner to repeat a unit's core move **on their own repository**, clearly marked as bring-your-own and non-verifiable per R7.AC8, for at least the Daily Driver stage's workflow units.
- R21.AC6 — All R21 content SHALL be taught **within the existing can-do taxonomy** (C1–C17 + CV mapped via existing units' front matter); it SHALL NOT add can-do statements or alter stages (the taxonomy decision is recorded in `decisions.md`; reopening it is a curriculum change outside this requirement).
...
## 8. Traceability
Each requirement ID (`R1`–`R21`) will be referenced by the corresponding component(s) in `design.md` and by tasks in `tasks.md`. No requirement may be dropped without an explicit, recorded decision.
