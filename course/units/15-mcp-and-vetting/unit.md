...
> them, and a project `.mcp.json` server stays **`⏸ Pending approval`** until you approve it ({{vd:mcp}}).
> The catch: connecting a server — or installing a plugin/marketplace item — is a **trust-delegation
> decision** (R10.AC5). It runs code or sees data for you, so **vet it first**: source, scope, transport,
> secrets ({{vd:plugins}}). This repo ships a real, offline stdio server you connect in the lab
...
**3 — Connecting is a trust-delegation decision.** This is the unit's thesis (R10.AC5). An MCP server,
...
not "looks fine." The version-specific surface for plugins/marketplace vetting is {{vd:plugins}}.
...
Connected`); confirm them against `claude mcp --help` before relying on a detail, and treat the
extension-vetting surface as {{vd:plugins}}. Tracked in
...
Confirm every command against {{vd:mcp}} rather than typing it from memory.
...
**Part B — vet (the judgment, R10.AC5):** consider a third-party MCP server you're tempted to install —
...
`⏸ Pending approval` gate ({{vd:mcp}}) lets you hold the decision until you've vetted.
...
- **Authoring the commands from memory.** `claude mcp` subcommands, transports, and the `.mcp.json`
  shape are version-specific — confirm against `claude mcp --help` ({{vd:mcp}}) rather than recall.
...
- The `mcp` subcommands and extension-vetting surface — {{vd:mcp}} and {{vd:plugins}};
