...
- **Guard the refactor against scope creep** — refuse to mix a behavior change (even a tempting
  bug-fix) into a "pure" refactor, and diff-review specifically for behavior that snuck in (cross-cutting verification, CV).
...
> the heavy version of **Explore a codebase**'s light explore) and **multi-file refactoring**
...
> one byte of observable behavior** — including the overdue bug from **Debugging**, which you must *not* fix here
...
**Part 1 — deep onboarding.** You met the light version in [Explore a codebase](../02-explore-a-codebase/unit.md):
...
**Part 2 — multi-file refactoring.** A refactor is a **behavior-preserving** change: the code's
...
is green." (It's the capture reflex from **Debugging** — pin behavior so a change is measurable — pointed at a whole
...
Plan mode (from **Ship a feature**) is the right gate here — get the decomposition plan and approve it *before* Claude
starts moving code.
...
("I'm right here, I'll just fix it") is exactly the scope creep this workflow warns about. **Don't.** A refactor
...
and its tests via the Bash tool, plan mode from **Ship a feature**). See
...
> refactor **preserved behavior exactly** (the "green before and after" rule, and it fails if you let a
...
2. **Onboard deeply.** Ask Claude to map the file: the layers (storage / domain / formatting /
...
4. **Plan the decomposition, then refactor in increments.** In plan mode, get a module split you
...
**Self-check (objective):** run `tools/verify-lab u09-lab1`. It passes only if, driving your
...
- **Next:** **Spec-driven dev** is the other force-multiplier discipline — building the *right*
  thing up front rather than rescuing the wrong thing later; **Code & security review** is how you'd
  review a refactor like this one before it merges.
- [`meta/workflows.md`](../../../meta/workflows.md#w5--multi-file-refactoring) — generalized multi-file
  refactoring, and [deep onboarding](../../../meta/workflows.md#w8--onboarding-to-an-unfamiliarlarge-codebase)
  (the heavy version of [Explore a codebase](../02-explore-a-codebase/unit.md)'s light explore).
- The characterization-test reflex is [Debugging](../07-debug-a-failure/unit.md)'s "capture it so 'fixed' is
  objective" / [TDD](../06-tdd/unit.md)'s red-green, scaled from one bug to a whole program — and the
  debugging lab literally foreshadowed this one: the overdue bug hid in three copies *because* of the
  duplication you untangle here.
