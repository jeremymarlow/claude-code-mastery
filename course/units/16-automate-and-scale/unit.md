...
Action wraps the headless invocation; the version-/integration-specific details are {{vd:ci}}. The key
...
**3 — Parallel agents via worktrees (W9).** When several **independent** tasks can proceed at once,
...
**Version currency.** Verified against Claude Code {{vd:_verified_version}}. `-p`/`--print`,
`--output-format`, `--max-budget-usd`, and `--worktree` are `--help`-verified; the GitHub Action
integration ({{vd:ci}}) and in-REPL rewind ({{vd:checkpoint-rewind}}) are external/in-REPL surfaces —
confirm them against the docs / `--help` before relying on a detail, rather than authoring from memory.
...
- **It also schedules a drift check** (`cron`) — a weekly trigger to re-verify version-specific facts
  against the CLI (R12.AC7). Automation isn't only for tests; here it's wired to prompt the very
...
# Parallel via worktrees (W9): two independent tasks, isolated trees, reviewed separately.
...
1. **Headless.** From `taskflow-api`, run a one-shot task with `claude -p "<task>" --output-format json`
   ...
   unattended run ({{vd:headless}}).
2. **Parallel via worktrees (W9).** Pick **two genuinely independent** changes (e.g. add a `GET
...
4. **CI.** Read [`.github/workflows/checks.yml`](../../../.github/workflows/checks.yml). Identify the
   trigger, what runs unattended, and how it relates to the U14 hook + pre-commit gate (defense in
...
- [ ] No flag or integration detail was **authored from memory** — confirmed against {{vd:headless}} /
      {{vd:worktrees}} / {{vd:ci}}.
...
[W9 method](../../../meta/workflows.md#w9--running-parallel-agents-git-worktrees) is the reference for
...
- **Parsing prose output in a script.** If something downstream consumes the result, use
  `--output-format json`/`stream-json` — scraping the human-readable text is brittle ({{vd:headless}}).
- **Authoring flags/Action config from memory.** Headless flags, the worktree flag, and the CI Action
  are version-/integration-specific — confirm against `--help` and the docs ({{vd:headless}},
  {{vd:worktrees}}, {{vd:ci}}).
