...
**The REPL.** `claude` starts an interactive session by default; `-p`/`--print` runs non-interactively (useful for pipes). You will live in the interactive session for daily work; the
...
**Awareness — IDE integration.** Claude Code also runs inside your editor: `--ide` auto-connects to an IDE on startup when exactly one valid IDE is available. The course
...
**Version currency.** This content was verified against Claude Code `2.1.195`. If
