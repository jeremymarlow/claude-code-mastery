...
> each with a `description` that tells the main agent when to reach for it ({{vd:subagents}}). The
...
on a detail ({{vd:subagents}}, tracked in [`meta/version-record.md`](../../../meta/version-record.md)).
...
**convention** — confirm the path/front-matter against the docs, {{vd:subagents}}), with that
...
> (R7.AC3/AC7). So the self-check is an objective checklist you apply to what you built and to the
...
authoring it from memory ({{vd:subagents}}).
...
- [ ] Nothing in the definition **hardcodes a version-specific format from memory** — the path and
      front-matter were confirmed against `claude --help`/docs ({{vd:subagents}}).
...
- **Authoring the agent format from memory.** The flags, the subcommand, and the on-disk path/front
  ...
  trusting recall ({{vd:subagents}}).

## Going deeper

- **Next in the Autonomy stage:** U14 (hooks) automates *enforcement* — and this repo's own `make check`
