...
`check-evaluations` reads **154/253 leaves across 14/23 sessions** (foundational, catalog-approval,
...
p6-capstone-finalization, l1-version-key-verification, quality-pass-8-lens-review-and-p7-start) and **1/23 syntheses** (foundational). In practice the
**leaf and synthesis stages run as separate passes** (`/evaluate-session` writes leaves only), so **13 leaf-complete
sessions await `_synthesis.md`**. The rest (those 13 syntheses + the remaining 9 sessions' leaves & syntheses,
...
fails on **R19** (unreferenced — design deferred, L12) **+ the incomplete corpus** (154/253 leaves, 1/23
