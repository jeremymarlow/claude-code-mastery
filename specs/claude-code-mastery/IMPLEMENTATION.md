...
`unverified`, blocked by enterprise access this environment lacks. (**L14 closed 2026-06-03**: the
`version-data.json` twin is now generated by `tools/render-vd-json` + gated in `make check`, decision
`P-vd-json-gen`.) See the 🔓 ledger in `decisions.md`.

**P9 — Collaboration retrospective (R18) ✅ COMPLETE (2026-06-02); breadcrumb nav (R19) → P10.**
R18 is fully built on `main` (panel + 253-leaf matrix + 23 syntheses + 11 globals + corner + learner case
study + U13 dogfood wiring); **R14.AC8** (disclose conflicts of interest) was added in-pass; R19 was
deferred at P9 close (L12) and has **since been built — see P10 below**. `make check` green; at P9 close
`make check-strict` failed only on R19 (now green). (Close-out committed + pushed on `main`.) Two new requirements via the post-v1
...
pilot **PASS** over the foundational session). **9.5 ✅ COMPLETE (2026-06-02) — leaf pass ✅ (253/253,
all 23 sessions) + per-session syntheses ✅ (23/23).** Leaf and synthesis stages ran as separate passes. **Workflow update (2026-06-01, `P9-leaf-workflow`):** the
...
(decision **P9-coi**). **9.8 ✅ DONE (2026-06-02, committed at 9.9):** the real 11-subagent panel is wired
...
**now passes** (253/253 leaves + 23/23 syntheses + 11 globals + corner). **R19** (breadcrumb nav for
learner docs) was an approved requirement with design deferred until R18 shipped — **built in P10, below**.
See `decisions.md` → "P9 …" / ledger **L11**–**L13** and `tasks/P9-collaboration-retrospective.md`.

**P10 — Breadcrumb navigation (R19) ✅ COMPLETE (2026-06-09).** Built spec-first on `main` via the
post-v1 playbook (design **§14** approved → `tasks/P10-breadcrumbs.md` → 5 slices). One shared trail rule
— `_common.breadcrumb`: hierarchy **derived from the filesystem** (ancestor chain = every `README.md`
walking up; repo-root README = home), labels = each ancestor's H1, final segment = the doc's own H1 as
plain text — emitted by all four generators (`render-units`/`render-index`/`render-cli-reference`/
`render-checklist`) and added to the hand-authored docs; enforced by **`tools/check-breadcrumbs`**
(hard fail, no PEND) in `make check`/`make check-strict`; convention recorded in `meta/conventions.md`
(which also names the four deliberate exemptions, incl. the home README as the trail terminus).
...
**P11 — Content enhancement (R20/R21) ✅ COMPLETE (2026-06-09).** Triggered by a five-persona
fresh-eyes review of all 16 units (`log/content-review/` — five reviewer files + synthesis) that
converged on five findings: the course never *showed* the tool in use, shipped rendering defects
(tool-call residue + spliced `{{vd}}` blobs), thin operator craft, register saturation, no
consolidation/transfer arc. Built spec-first in one pre-authorized run (decision `P11-delegation`):
**R20** (every core unit demonstrates its skill via the §15.1 Captured/Illustrative convention — all
captures real and regenerable) and **R21** (brief craft in U5, session lifecycle + recovery in U4
reinforced in U9, stage checkpoints in U4/U8/U11/U16, Daily-Driver transfer blocks) plus E2
(`inline` vd form + per-doc dedupe + `tools/check-content` residue/demo gate) and a moderate E3
register pass, woven one-touch-per-unit. An 11.8 fresh-eyes spot review verified S1 closed and
caught one factual error in U7's dialogue (fixed; decision `P11-spotreview`). **`make check-strict`
...
_(The "remaining open" items that once sat here — repo structure §9, awareness/core edge cases,
capstone brief menu — were all resolved as Design completed and was approved on 2026-05-29; the
brief menu landed in P6 (ledger L5). Nothing in this section is open. Current state lives in §3.)_
