...
- **Write and place a `CLAUDE.md` that measurably changes how Claude behaves on a task** — encode a
  project's conventions once and have Claude follow them — advances `C5`.
- **Treat the context window as a managed resource** — inspect what's loaded, select what's relevant,
  and clear what isn't — advances `C5`.
- **Locate where settings live** (project vs user) and recognize that the permission rules from U3
  are configured here — advances `C5`.
- **Verify that your memory/context change actually took effect** before trusting the behavior change —
  the `CV` reflex applied to configuration.
...
> is. Inspect it, select into it (`@`-mentions, from U2), and clear it when you switch tasks. The lab
...
You've now driven a change (U1), explored code (U2), and operated safely (U3). What you haven't done
...
**1 — Project memory (`CLAUDE.md`).** {{vd:memory}} A `CLAUDE.md` is a file Claude auto-discovers and
loads as standing instructions — your project's conventions, architecture, and gotchas, stated once.
The baseline you were handed in U1 (`codebases/primary/taskflow-api/CLAUDE.md`) is a working example;
...
- **More context is not better context.** Dumping the whole repo "to be safe" crowds out the few
  files that matter and degrades answers. **Select deliberately:** anchor with `@`-mentions (U2),
  start from the right directory, and don't drag in what the task doesn't need.
- **Inspect and reset it.** {{vd:context-cmds}} You can see what's currently loaded, compact a long
...
*(Enterprise/managed settings can override these — see U3.)*

> **Awareness — output styles.** Claude's response style/verbosity is itself configurable:
> {{vd:output-styles}} Useful to know it exists; not something this course drills.
...
The repo's root [`CLAUDE.md`](../../../CLAUDE.md) is the authentic specimen (R14.AC1). Notice what it
does and doesn't do:
...
3. **Confirm the file is loaded (CV).** Inspect the context ({{vd:context-cmds}}) and verify the
...
- **Next:** U5 (Ship a feature) puts this to work — a sharp `CLAUDE.md` plus plan mode is what makes
...
- The `@`-mention selection habit from [U2](../02-explore-a-codebase/unit.md) is the context lever in
  miniature; the permission *postures* from [U3](../03-operate-safely/unit.md) are configured in the
  `settings.json` discussed here.
