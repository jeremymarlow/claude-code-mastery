...
You can now land a change (U1), read unfamiliar code (U2), operate safely (U3), and steer Claude with
memory and context (U4). This unit is where those combine into the **daily-driver loop** — the one you'll
...
**The workflow — W1.** The generalized pattern lives once in
...
to you** — the habit from U2, now load-bearing. For the lab's stats endpoint that means: where do routes
...
**2 — Plan mode is the cheapest place to be wrong.** {{vd:plan-mode}} In plan mode Claude investigates
...
[`CLAUDE.md`](../../../codebases/primary/taskflow-api/CLAUDE.md), from U4) — catch that in the plan and
...
This is the same diff-reading reflex from U1, now on a change big enough that it earns its keep.

> **Awareness — extended thinking.** For a genuinely hard plan — a thorny design trade-off, a subtle
> algorithm — you can have Claude think harder before answering: {{vd:thinking}} It's a dial you reach
...
**Version currency.** This unit was verified against Claude Code `{{vd:_verified_version}}`; plan mode and
...
any unit is authored (that's the heavy-weight cousin of W1, taught fully in U10). The everyday version is
...
2. **Explore.** Ask Claude to read the task and project routers/services and restate: where a new route
   ...
   against the files (U2 habit) — the plan is only as good as this.
...
- **Letting logic land in the router.** The baseline `CLAUDE.md` says domain logic lives in the service
  ...
  in review. (This is also why U4 mattered: memory only steers if you let it.)
...
- **Committing on green tests alone.** Tests pass ≠ change is correct and scoped. Read the diff every
  time — the habit you built on one line in U1 is the same habit here, where it actually protects you.
...
- **Next:** U6 (TDD) tightens the loop by writing the failing test *first*; U7 (debugging) and U8 (Git/PR)
  build on this same explore→plan→code→commit spine. Everything in the Daily Driver stage assumes you can
  run W1 fluently.
- [`meta/workflows.md`](../../../meta/workflows.md#w1--explore--plan--code--commit) — the generalized W1
