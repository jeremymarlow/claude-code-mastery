...
version-specific surface is the `gh` integration and PR-linked session resume: `--from-pr` session resume; PR creation via `gh` in the Bash tool Confirm with
...
- The `gh` integration and PR-linked session resume: `--from-pr` session resume; PR creation via `gh` in the Bash tool Version-specifics in
