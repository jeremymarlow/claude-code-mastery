...
|---|---|---|---|
| 2.1.195 | 2026-06-26 | **version refresh** 2.1.170→2.1.195 (25 releases) **+ quarantine-gap closure**: one new top-level command (`gateway`) and deeper-surface additions, all captured by the regenerated `cli-reference.{json,md}`; cumulative `version-changelog.md` entry cites the official CHANGELOG (**9 versions absent upstream — marked**; **4 introspection-only surface changes** — `gateway`, `--allowed`/`--disallowed`, `--ax-screen-reader`, removed `--allowedTools`/`--disallowedTools`/`--mcp-debug` — recorded from `--help`, not the changelog; `--bg`/`--background` were a fixed `--help` omission, not new). Every `{{vd:key}}` re-checked vs the installed CLI — **none contradicted** (`--permission-mode`/`--effort`/`--output-format` byte-identical); `mcp` re-verified live and extended (`login`/`logout`, `✓ Connected`). **Added 4 keys** closing audit-found quarantine gaps — `skip-permissions`, `hook-output` (docs-verified), `cli-help`, extended `mcp` — and re-tokenized literal-beside-token leakage across U3/U11/U13/U14/U15/U16. `managed-settings` stays `unverified` (L1: new enterprise `gateway`/`enforceAvailableModels` surface noted, but path/precedence still needs enterprise access). `_verified_version` bumped → units re-rendered | version refresh + quarantine closure |
...
- `managed-settings` — the enterprise/managed-settings **file path + precedence** needs an enterprise
  account or official enterprise docs (not available to the maintainer). The 2.1.170→2.1.195 refresh
  surfaced **new adjacent enterprise surface** — the `gateway` command (auth/telemetry) and the
  `enforceAvailableModels`/`availableModels` managed settings — but none reveals the managed-settings
  file path or precedence, so L1 stays blocked.
