...
In U5 and U6 you *added* behavior; debugging is the inverse skill — something already works wrong and
...
**The workflow — W3.** The generalized pattern lives once in
...
is the W2 reflex (U6) pointed at a bug: the captured failure is what flips from red to green to *prove*
the fix, and it guards the bug from silently coming back. (The legacy CLI has no test suite at all —
which means the repro you capture here is its *first* characterization test, a thread U9 picks up.)
...
confirming it before editing is what kept the check honest. (That exact failure happened authoring U6.)
...
- **Next:** U8 (Git/PR) turns the verified fix into reviewable history; U9 (onboard + refactor) returns
